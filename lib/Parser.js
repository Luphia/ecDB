(function(exports) {
	...
	exports.sql2ast = function (query, parseCond) {
		...
		var keywords = ['SELECT', 'FROM', 'DELETE FROM', 'INSERT INTO', 'UPDATE', 'JOIN', 'LEFT JOIN', 'RIGHT JOIN', 'INNER JOIN', 'OUTER JOIN', 'ORDER BY', 'GROUP BY', 'HAVING', 'WHERE', 'LIMIT', 'VALUES', 'SET'];
		...
		analysis['LEFT JOIN'] = analysis['JOIN'] = analysis['INNER JOIN'] = analysis['OUTER JOIN'] = analysis['RIGHT JOIN'] = function (str) {
			str = str.split(/ ON /i);
		...
		if (typeof result['OUTER JOIN'] != 'undefined') {
			if (typeof result['JOIN'] == 'undefined') result['JOIN'] = [];
			if (typeof result['OUTER JOIN'][0] != 'undefined') {
				result['OUTER JOIN'].forEach(function (item) {
					item.type = 'outer';
					result['JOIN'].push(item);
				});
			}
			else {
				result['OUTER JOIN'].type = 'outer';
				result['JOIN'].push(result['OUTER JOIN']);
			}
			delete result['OUTER JOIN'];
		}
